...
func DUTInfo() (testbench.DUTInfo, error) {
	_, ctrlIPv4, _, err := netdevs.IfaceInfo(dut.CtrlIface)
	if err != nil {
		return testbench.DUTInfo{}, err
	}
	testLink, testIPv4, testIPv6, err := netdevs.IfaceInfo(dut.TestIface)
	if err != nil {
		return testbench.DUTInfo{}, err
	}
	dutUname, err := uname()
