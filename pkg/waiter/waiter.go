...
func (e *Entry) NotifyEvent(mask EventMask) {
	if m := mask & e.mask; m != 0 {
		e.eventListener.NotifyEvent(e.mask & mask)
	}
}
...
//
// In general, this should only be used for tests.
...
// in common with the notification mask. This function returns the number of
// notifications delivered.
func (q *Queue) Notify(mask EventMask) (count int) {
	q.mu.RLock()
