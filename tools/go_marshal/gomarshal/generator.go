...
func (g *Generator) generateOne(t *marshallableType, fset *token.FileSet) *interfaceGenerator {
	...
	if t.dynamic {
		...
		if t.boundCheck {
			abortAt(fset.Position(t.slice.comment.Slash), "Can not generated Checked methods for dynamic types. Has to be implemented manually.")
		}
	...
	case *ast.Ident:
		...
		if t.boundCheck {
			i.emitCheckedMethodsForPrimitiveNewtype()
		}
		if t.slice != nil {
	...
	case *ast.ArrayType:
		...
		if t.boundCheck {
			i.emitCheckedMethodsForArrayNewtype()
		}
		if t.slice != nil {
