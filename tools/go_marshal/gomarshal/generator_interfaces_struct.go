...
func (g *interfaceGenerator) emitCheckedMethodsForStruct() {
	...
	g.inIndent(func() {
		...
		g.inIndent(func() {
			g.emit("return nil, false\n")
		})
		g.emit("}\n")
		g.emit("// MarshalUnsafe will fallback to MarshalBytes if need be.\n")
	...
	g.inIndent(func() {
		...
		g.emit("}\n")
		g.emit("// UnmarshalUnsafe will fallback to UnmarshalBytes if need be.\n")
